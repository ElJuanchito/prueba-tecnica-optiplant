...
| Field | Value |
|-------|-------|
| Estimated changed lines | ~2 550 total (see per-unit table) |
...
| S6. Product units | `ProductUnitPolicy` → web, incl. the default-swap ordering | 400 | `cd backend && ./mvnw test -Dtest=ProductUnitPolicyTest` | `./mvnw verify -Dit.test=ProductUnitCatalogIT` | revert commit; units subresource disappears |
...
| S8. Cross-cutting | RBAC/401 IT, no-numeric-id assertion, audit atomicity, DT-07, counters | 240 | — | `./mvnw verify` + both scripts | revert commit; guarantees unproven |
...
- [ ] 6.4 Create `ProductUnitSpringDataRepository.java` with `clearDefaultSaleUnit` as `@Modifying(flushAutomatically = true, clearAutomatically = true)` bulk JPQL update (design §8.2, D-11).
- [ ] 6.5 Create `ProductUnitPersistenceAdapter.java` — on any write that sets `is_default_sale_unit = TRUE`, call `clearDefaultSaleUnit` **first**. `uq_product_units_single_default` is a partial unique index and PostgreSQL cannot defer it, so the intermediate two-`TRUE` state aborts the transaction (design §8.2).
- [ ] 6.6 Create `catalog/infrastructure/adapter/in/web/ProductUnitController.java` — the four endpoints of contract §6.3; collection **not** paginated (justified exception to RNF-PER-04); a unit belonging to another product returns `404`, never `200`; `DELETE` returns `204`.
- [ ] 6.7 Extend `CatalogExceptionHandler` with `ProductUnitNotFoundException` → `404 product_unit_not_found`, `DuplicateProductUnitException` → `409`, and the `uq_product_units_single_default` branch of `DataIntegrityViolationException` → `409` with a hand-written message that names no constraint (§7.1 point 2).
- [ ] 6.8 Test: `ProductUnitPolicyTest` — factor `0` and `-1` rejected; base-unit homonym with factor ≠ 1 rejected and with factor 1 accepted; marking a new default leaves exactly one marked; removing the default leaves none; duplicate `unitName` rejected (R-13, R-14).
- [ ] 6.9 **[BLOCKING]** Test IT: `ProductUnitCatalogIT` — swap the default from `SACO_50KG` to `BULTITO_10KG` against real PostgreSQL and confirm it commits with exactly one default. A unit test cannot catch this: it is a Hibernate flush-ordering fact of the real database (design §8.2).
- [ ] 6.10 Test IT: two different products each mark their own default and both succeed (the index is scoped by `product_id`); a product with no default reads back fine; a direct repository write of a second default surfaces as a `409`, not a `500` (R-14, S-3).
- [ ] 6.11 Test IT: deleting a unit affects no balance or movement; a unit id belonging to another product returns `404`.
- [ ] 6.12 Run `cd backend && ./mvnw verify`.
...
- [ ] 8.8 Add `DT-07 — Exposición HTTP del cambio de unidad base, diferida` to `docs/deuda_tecnica.md`: a `**DT-07**` row in the summary table **and** a `### DT-07` ficha, in Spanish, with the text of design §11.1. Both are required — `validar_trazabilidad.py:83-86` fails a declared DT with no ficha.
...
- [ ] 8.13 Confirm the corrected DoD item (design §10.4, D-12): no `RF`/`RNF`/`RN` identifier was created and `docs/casos_de_uso.md`'s traceability matrix is untouched. `git diff --stat docs/` is **not** empty by design — it shows `diagrama_er.md` (1.8) and `deuda_tecnica.md` (8.8), both deliberate.
